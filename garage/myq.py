...
def myq_main2(command, device="none"):
    ...
    if command == 'status':
        ...
        if command == 'close' and device != "none":
            device_type = 'door'
            desired_state = 0
        elif command == 'open' and device != "none":
            device_type = 'door'
            desired_state = 1
        elif command == 'off' and device != "none":
            device_type = 'light'
            desired_state = 0
        elif command == 'on' and device != "none":
            device_type = 'light'
            desired_state = 1
        else:
            show_usage()
    ...
    if desired_state == 2:
        for inst in device_instances:
            #print ('{} is {}. Last changed at {}'.format(inst.name, inst.state, inst.time))
            msg = ('{} is {}. Last changed at {}'.format(inst.name, inst.state, inst.time))
            LOGGER.info('%s is %s. Last changed at %s', inst.name, inst.state, inst.time)
            if USE_ISY:
                id, varname, init, value = isy_get_var_id(inst.name)
                value = 1 if inst.state in ["Open","On"] else 0
        ...
        for inst in device_instances:
            #name = " ".join(device)
            if device.lower() == inst.name.lower():
                myq.set_state(inst, device_type, desired_state)
                if USE_ISY:
                    id, varname, init, value = isy_get_var_id(inst.name)
                    isy_set_var_state(id, inst.name, varname, desired_state)
                #print(inst.name + ' found. Setting state to ' + command)
                msg = (inst.name + ' found. Setting state to ' + command)
                success = True
        if not success:
            #print(name + ' not found in available devices.')
            msg = (name + ' not found in available devices.')
    return msg
